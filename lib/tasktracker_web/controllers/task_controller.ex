...
#    tasktracker_params = for {key, val} <- tasktracker_params, into: %{}, do: {String.to_atom(key), val}
    ...
    case changeset.valid? do
      true -> IO.puts "changeset is valid"
              case TaskManager.create_task(task_params |> Map.delete(:timetrackers)) do
                {:ok, task} ->
      ...
      false ->
#        changeset = Task.changesetWithTimetracker(task = %Task{timetrackers: [%Timetracker{}]}, task_params)
        IO.puts "timetracker changeset is not valid"
